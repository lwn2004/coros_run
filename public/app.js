...
let currentCalDisplay = 'calendar'; // 'calendar' | 'chart'
let currentCalDate = new Date(); 
let calYearChartInstance = null;
let calMonthChartInstance = null;
...
function getMonthCellColor(dist) {
    if (dist === 0) return 'transparent';
    const accentHex = localStorage.getItem('accentColor') || '#e93342';
    if (dist >= GOAL_MONTHLY) return accentHex; 
    
    const minOpacity = 0.15;
    const maxOpacity = 0.9;
    const opacity = minOpacity + (dist / GOAL_MONTHLY) * (maxOpacity - minOpacity);
    return hexToRgbaFast(accentHex, opacity);
}
...
function updateCalendarCard() {
    ...
    const isCal = currentCalDisplay === 'calendar';

    document.querySelectorAll('.cal-tab').forEach(t => t.classList.remove('active'));
    document.querySelector(`.cal-tab[data-mode="${currentCalMode}"]`).classList.add('active');

    document.getElementById('cal-view-month-cal').style.display = (isMonth && isCal) ? 'flex' : 'none';
    document.getElementById('cal-view-month-chart').style.display = (isMonth && !isCal) ? 'block' : 'none';
    document.getElementById('cal-view-year-cal').style.display = (!isMonth && isCal) ? 'flex' : 'none';
    document.getElementById('cal-view-year-chart').style.display = (!isMonth && !isCal) ? 'flex' : 'none';

    document.getElementById('btn-view-cal').classList.toggle('active', isCal);
    document.getElementById('btn-view-chart').classList.toggle('active', !isCal);
    ...
    if (isMonth) {
        if (isCal) renderMonthGrid(year, month, dayMap);
        else renderMonthChart(year, month, dayMap);
    } else {
        if (isCal) renderYearGrid(year, monthMap);
        else renderYearChart(monthMap);
    }
...
function renderYearGrid(year, monthMap) {
    ...
    monthMap.forEach((dist, i) => {
        const cell = document.createElement('div');
        cell.className = 'cal-month-cell';
        if (dist > 0) {
            cell.classList.add('has-data');
            cell.style.backgroundColor = getMonthCellColor(dist);
            if (dist >= GOAL_MONTHLY) cell.style.color = '#fff';
            
            cell.innerHTML = `<div>${months[i]}</div><div class="dist" ${dist >= GOAL_MONTHLY ? 'style="color:rgba(255,255,255,0.9)"' : ''}>${dist.toFixed(1)} km</div>`;
            
            cell.onclick = () => {
                currentCalDate = new Date(year, i, 1);
            ...
            cell.classList.add('empty');
            cell.innerHTML = `<div>${months[i]}</div><div class="dist">0 km</div>`;
        }
        grid.appendChild(cell);
    });
}
...
// ==========================================================================
// AI Guide Card Logic (Dashboard Preview)
// ==========================================================================
async function loadAiGuidePreview() {
    const previewEl = document.getElementById('ai-summary-preview');
    if (!previewEl) return;
    try {
        const res = await fetch('https://run.linwn.net/data/summary/index.json');
        if (res.ok) {
            ...
            if (list.length > 0) {
                const mdRes = await fetch(`https://run.linwn.net/data/summary/${list[0].report}`);
                if (mdRes.ok) {
                    const mdText = await mdRes.text();
                    let plainText = mdText.replace(/^#+\s+/gm, '') 
                                          .replace(/[*_`]/g, '')
                                          .replace(/\[([^\]]+)\]\([^\)]+\)/g, '$1') 
                                          .replace(/\n+/g, ' ')
                                          .trim();
                    previewEl.innerText = plainText;
                } else {
                    previewEl.innerText = "未能加载摘要内容。";
                }
            } else {
                previewEl.innerText = "暂无 AI 分析数据。";
            }
        }
    } catch (err) {
        console.error("Failed to load AI guide preview", err);
        previewEl.innerText = "摘要加载失败。";
    }
}
...
document.querySelectorAll('.filter-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
        if(e.target.id === 'settings-close' || e.target.id === 'settings-save' || e.target.id === 'pb-toggle-btn' || e.target.id === 'btn-view-cal' || e.target.id === 'btn-view-chart' || e.target.id === 'ai-btn-prev' || e.target.id === 'ai-btn-next') return;
